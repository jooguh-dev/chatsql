...
def test_ai_configuration():
    ...
    # 检查配置
    openai_mode = getattr(settings, 'OPENAI_MODE', 'mock')
    api_key = os.getenv('OPENAI_API_KEY')
    
    print(f"\n1. OPENAI_MODE: {openai_mode}")
    print(f"2. OPENAI_API_KEY: {'已配置' if api_key else '未配置'}")
    if api_key:
        print(f"   API Key 前缀: {api_key[:20]}...")
    
    if openai_mode != 'real':
        print("\n⚠️  警告: OPENAI_MODE 不是 'real'，AI将使用mock模式")
        print("   请在 .env 文件中设置: OPENAI_MODE=real")
        return False
...
def test_ai_with_submissions():
    ...
    try:
        ...
        if 'submission' in response_text or 'incorrect' in response_text:
